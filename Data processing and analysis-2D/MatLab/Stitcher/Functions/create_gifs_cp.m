function create_gifs_cp(File, Settings);
...
for experiment_n = 1: size(temp.experiment,1) %loop over experiments
    ...
    for well_n = 1:size(temp.wells,1) %loop over wells
        ...
        for k= 1: size(list_files,1)-2
            ...
            filename_im3 = horzcat( char( unique(File.files_list_s.folder_processed_experiment)), ...
                '2 - CellProfiler results/Nuclei/Nuc0', num2str(k_cp,'%03.f'),...
                '.jpeg');
            ...
            im_t = uint8(cat(3,im_n,im_c.*Settings.im_c_correctionfactor,zeros(size(im_n))));
            ...
            filename = horzcat( char( unique(File.files_list_s.folder_processed_experiment)), ...
                '2 - CellProfiler results/all_images_well', num2str(well_n),'.gif');
